...
export class VonageRequest extends JovoRequest {
    ...
    getInputText(): JovoInput['text'] {
        return (
            this.speech?.results?.filter((r) => r.confidence > 0.5)[0]?.text ?? this.dtmf?.digits
        );
    }
    ...
    getIntent(): JovoInput['intent'] {
        ...
        return this.speech?.timeout_reason &&
            this.speech?.timeout_reason !== 'end_on_silence_timeout'
            ? 'silence'
            : undefined;
    }
